...
def ExtractRamPointer(byteList: [bytes]):
    pointer = 0
    for a in range(len(byteList)):
        pointer += (int(byteList[a])) << (8 * a)
...
def main():
    ...
    with open(ROM_NAME, 'rb+') as rom:
        ...
        # Do Special Inserts
        if os.path.isfile(BYTE_CHANGES) and os.path.isfile(BYTE_CHANGES_OUT):
            with open(BYTE_CHANGES, 'r') as file:
                offsetList = []
                for line in file:
            ...
            with open(BYTE_CHANGES_OUT, 'rb') as binFile:
